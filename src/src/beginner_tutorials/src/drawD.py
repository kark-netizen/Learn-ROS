...
def forward(velocity_publisher, line_vel, dist):
    ...
    while True:
        ...
        velocity_publisher.publish(line_vel)
        rate.sleep()
...
def Orientation(publisher, speed_in_degree, desired_angle_degree):
...
def letterD(velocity_publisher):
    Orientation(velocity_publisher, 45, 90)
    forward(velocity_publisher, 1, 8, True)
    Orientation(velocity_publisher, 45, 0)
